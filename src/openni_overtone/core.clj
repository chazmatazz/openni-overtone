...
(defn make-trapezoidal-box [pts]
  (let [[p1 p2 p3 p4 p5 p6 p7 p8] pts]
         [[p1 p2] [p1 p3] [p1 p5] [p2 p4]
          [p2 p6] [p3 p4] [p3 p7] [p4 p8]
          [p5 p6] [p5 p7] [p6 p8] [p7 p8]]))

(defn make-pyramid [pts]
  (let [[p1 p2 p3 p4 p5] pts]
    [[p1 p2] [p1 p3] [p1 p4] [p1 p5]
     [p2 p3] [p4 p5] [p2 p4] [p3 p5]]))

(defn make-cycle [pts]
  (into [] (for [i (range (count pts))]
             [(nth pts i) (nth pts (mod (+ i 1) (count pts)))])))

(defn get-pts-from-joints [joints]
  (reduce (fn [acc joint] (conj (conj acc (first joint)) (second joint))) (set {}) joints))

(defn complete-graph [pts]
  (into [] (for [p pts q pts]
             [p q])))

(def arm-leg-trapezoidal-box-joint-pairs (make-trapezoidal-box
                                  [:right-elbow :right-hand :left-elbow :left-hand
                                   :right-hip :right-knee :left-hip :left-knee]))

(def head-pyramid-joint-pairs (make-pyramid
                               [:head :right-elbow :right-hand :left-elbow :left-hand]))

(def arm-cycle-joint-pairs (make-cycle
                            [:left-hand :left-elbow :left-shoulder
                             :right-shoulder :right-elbow :right-hand]))

(def complete-human-joint-pairs (complete-graph (get-pts-from-joints human-joint-pairs)))

(def extremities-cycle-joint-pairs (make-cycle
                                    [:head :left-hand :left-foot :right-foot :right-hand]))

(def joint-pairs-list (atom [{:joint-pairs human-joint-pairs,
                              :on true,
                              :stroke-weight 1,
                              :color [255 0 180]}
                             {:joint-pairs arm-leg-trapezoidal-box-joint-pairs
                              :on true,
                              :stroke-weight 3,
                              :color [255 255 0]}
                             {:joint-pairs head-pyramid-joint-pairs
                              :on false,
                              :stroke-weight 1,
                              :color [0 255 0]}
                             {:joint-pairs arm-cycle-joint-pairs
                              :on false,
                              :stroke-weight 3,
                              :color [0 255 0]}
                             {:joint-pairs complete-human-joint-pairs
                              :on false,
                              :stroke-weight 1,
                              :color [0 255 0]}
                             {:joint-pairs extremities-cycle-joint-pairs
                              :on false,
                              :stroke-weight 3,
                              :color [0 255 0]}]))
...
(def screen-mode (atom :performance))
...
(defn on-frame [the-key the-ref old-frame-count new-frame-count]
  ...
  (let [keyword-uid-skeletons (keywordize @bifocals/skeletons)]
    ...
    (when-not (= @screen-mode :off)
      ...
      (quil/display-filter :invert)
      (doseq [[keyword-uid skel] keyword-uid-skeletons]
              (let [project-skeleton (bifocals/project-skeleton skel)]
                (doseq [joint-pairs @joint-pairs-list]
                  (when (:on joint-pairs)
                    (quil/stroke-weight (:stroke-weight joint-pairs))
                    (let [[r g b] (:color joint-pairs)]
                      (quil/stroke r g b))
                    (doseq [joint-pair (:joint-pairs joint-pairs)]
                      (draw-line
                  ...
                  (let [joint (nth ctl-joints i)
                    ...
                    (if (= 0 joint-vol)
                      ...
                      (let [y-stage-range (joint joint-y-stage-range)
                            ...
                            pos-vec-y
                            (into []
                                  (for [skel skel-hist] (:x (joint skel))))
...
;; screen modes
;; initial state (= @screen-mode :performance)
;; @screen-mode
;; (reset! screen-mode :debug)
;; (reset! screen-mode :off)
;; (reset! screen-mode :performance)

;; ctl-modes
;; initial state (= @ctl-type :all)
;; @ctl-type
